 ...
 * File: js/shared/cpu/z80/instructions/Z80DataTransfer.js
 * 
 * Role:
 ...
 *    register loading, stack push/pop, and atomic exchange operations (EX, EXX).
 *    It has no knowledge of memory speed cycles or instruction fetches.
 ...
 *    entire, heavy execution loop of the ZilogZ80 class.
 */
...
class Z80DataTransfer {
    ...
    static register(cpu, registers, alu, registry) {
        ...
        registry.indexedIY[0x74] = [() => { cpu.theMMU.writeAddr(getDisplacement(registers.iy), registers.h); cpu.incPc(3); }, "LD (IX+%d),H", 19, 1, false];
