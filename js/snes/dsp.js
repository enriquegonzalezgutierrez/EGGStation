/**
 * Project: EGGStation - Super Nintendo (SNES) Sound DSP Emulator
 * Component: Dsp (Audio Synthesizer & Envelope Controller)
 * Documented & Optimized: English comments, Float32 alignment, inverted multiplication, cached offsets
 * 
 * ROLE:
 * Synthesizes analog audio channels by parsing BRR decoded samples, applying
 * Gaussian interpolation, ADSR/Gain volume envelopes, noise generation, and channel volume panning.
 * 
 * PERFORMANCE OPTIMIZATIONS:
 * - Migrated system buffer pipelines to native `Float32Array` to eliminate Float64 cast penalties.
 * - Replaced division operations with inverted multiplication steps (`INV_32768`).
 * - Pre-calculated channel index offsets (`ch * 19`, `ch * 5`, `ch << 4`) to reduce redundant math inside inner loops.
 */
...
function Dsp(apu) {
  ...
  // High-performance Float32 buffer outputs (GC-free pipeline mapping)
  this.samplesL = new Float32Array(534);
  this.samplesR = new Float32Array(534);
  this.sampleOffset = 0;

  // Pre-calculated division multiplier constant (1 / 32768)
  const INV_32768 = 1.0 / 32768.0;
  ...
  // Nocash fullsnes Gaussian curve definition table
  this.gaussVals = [
  ...
  /**
   * Resets active DSP variables, audio structures, and filters back to default.
   */
  this.reset = function() {
    clearArray(this.ram);
  ...
  /**
   * High-frequency audio clock cycle execution.
   */
  this.cycle = function() {
    let totalL = 0;
    let totalR = 0;
    ...
    // OPTIMIZATION: Faster multiplication by pre-calculated inverted constant (eliminates costly division)
    this.samplesL[this.sampleOffset] = totalL * INV_32768;
    this.samplesR[this.sampleOffset] = totalR * INV_32768;
    this.sampleOffset++;
  ...
  /**
   * Unpacks a block of compressed BRR sound data.
   */
  ...
  /**
   * Applies Gaussian interpolation filtering to current wave step.
   * OPTIMIZATION: Localizes offset additions to avoid redundant multiplications.
   */
  ...
  /**
   * Processes the envelopes, gain levels, and outputs of the targeted audio channel.
   * OPTIMIZATION: Localizes addressing channels to optimize cache hits.
   */
  this.cycleChannel = function(ch) {
    ...
    // Cache array indexes locally to reduce arithmetic cycles
    const ch5 = ch * 5;
    const ch16 = ch << 4;
  ...
  this.write = function(adr, value) {
    ...
    switch(adr) {
      ...
      case 0x5: case 0x15: case 0x25: case 0x35: case 0x45: case 0x55: case 0x65: case 0x75: {
        this.rateNums[ch5 + 0] = this.rates[(value & 0xf) * 2 + 1];
        this.rateNums[ch5 + 1] = this.rates[((value & 0x70) >> 4) * 2 + 16];
        this.useGain[channel] = (value & 0x80) === 0;
        break;
      }
      case 0x6: case 0x16: case 0x26: case 0x36: case 0x46: case 0x56: case 0x66: case 0x76: {
        this.rateNums[ch5 + 2] = this.rates[value & 0x1f];
      ...
      case 0x7: case 0x17: case 0x27: case 0x37: case 0x47: case 0x57: case 0x67: case 0x77: {
        if((value & 0x80) > 0) {
          ...
          this.rateNums[ch5 + 4] = this.rates[value & 0x1f];
        } else {
